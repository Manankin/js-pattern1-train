...
class Version {
  ...
  static #versions = [];
  // Статичний метод create приймає аргумент content (текст версії) і створює новий екземпляр класу Version в який передає content. Створений екземпляр додається до масиву версій versions.
  static create(content) {
    // console.log(content);
    this.#versions.push(new Version(content));
    // console.log(this.#versions)
  }
  // Статичний метод restore видаляє останній елемент масиву, та повертає останню збережену версію тексту з масиву версій this.#versions[this.#versions.length - 1].
  static restore() {
    this.#versions.pop();
